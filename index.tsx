...
// --- TYPE DEFINITIONS --- //
type View = 'dashboard' | 'tank-details' | 'decision-support' | 'settings' | 'financial-reports' | 'breeding-hub' | 'sustainability-report';
type FishAsset = typeof breedingAssets[0];
type IotDevice = { id: string, type: string, name: string };
type TankLayout = { 
    id: number;
    status: string;
    name: string;
...
const Icon: FC<{ name: string; className?: string; title?: string }> = ({ name, className, title }) => <i className={`fa-solid ${name} ${className}`} title={title}></i>;
...
const KpiCard: FC<{ kpi: { title: string, value: string, change?: number, positive?: boolean } }> = ({ kpi }) => {
...
const AlertCard: FC<{ alert: typeof alerts[0]; onSelectTank: (tankId: number) => void; }> = ({ alert, onSelectTank }) => {
    const colors = {
...
const Dashboard: FC<{ onSelectTank: (tankId: number) => void; setView: (view: View) => void }> = ({ onSelectTank, setView }) => {
    return (
...
const TankDetails: FC<{ tankId: number; onBack: () => void }> = ({ tankId, onBack }) => {
...
const PriceChart: FC<{ data: typeof priceTrends['畢卡索'], color: string, predictedColor: string }> = ({ data, color, predictedColor }) => {
    const maxPrice = Math.max(...data.map(d => d.price)) + 2;
    const minPrice = Math.min(...data.map(d => d.price)) - 2;
    const pointToCoords = (d: {price: number}, i: number) => `${i * 40},${80 - ((d.price - minPrice) / (maxPrice - minPrice)) * 80}`;
...
const DecisionSupport: FC<{ onBack: () => void }> = ({ onBack }) => {
    const [tab, setTab] = useState('efficiency');
    const [selectedBatchId, setSelectedBatchId] = useState(batchProfitabilityWithFCR[0].id);
    const [selectedFeedId, setSelectedFeedId] = useState(feedOptions[0].id);
...
const FarmSetup: FC = () => {
    const [layout, setLayout] = useState<TankLayout[]>(initialFarmLayout);
    const [unassignedDevices, setUnassignedDevices] = useState<IotDevice[]>(initialIotDevices);
    const [selectedTank, setSelectedTank] = useState<TankLayout | null>(null);
    ...
    const handleDragStart = (e: React.DragEvent, device: IotDevice) => {
        e.dataTransfer.setData("application/json", JSON.stringify(device));
    };

    const handleDrop = (e: React.DragEvent, targetTankId: number) => {
        e.preventDefault();
    ...
    const handleTankUpdate = (e: React.ChangeEvent<HTMLInputElement>) => {
...
const UserManagement: FC = () => {
    ...
    const [roles, setRoles] = useState<Record<string, Record<string, boolean>>>(initialRolesConfig);
    const [activeTab, setActiveTab] = useState('users');
    const [selectedRole, setSelectedRole] = useState('管理者');
...
const AiConfig: FC = () => {
    ...
    const [watchedDiseases, setWatchedDiseases] = useState({
        '白點病': true,
        '爛鰭病': true,
        '錨蟲病': false,
        '車輪蟲病': true,
    ...
    const [feedCost, setFeedCost] = useState(1.8);
    const [profitAlertThreshold, setProfitAlertThreshold] = useState(15);
...
const DataSettings: FC = () => {
    const [apiKey, setApiKey] = useState('aqua_sk_****************************1234');
...
const Settings: FC<{ onBack: () => void }> = ({ onBack }) => {
    const [page, setPage] = useState('users');
    const NavLink: FC<{pageName: string; icon: string; text: string}> = ({pageName, icon, text}) => (
...
const FinancialReports: FC<{ onBack: () => void }> = ({ onBack }) => {
...
const FishCard: FC<{ fish: FishAsset; onDragStart: (e: React.DragEvent, fish: FishAsset) => void }> = ({ fish, onDragStart }) => (
...
const BreedingAssetHub: FC<{ onBack: () => void }> = ({ onBack }) => {
    const [pair, setPair] = useState<[FishAsset | null, FishAsset | null]>([null, null]);

    const handleDragStart = (e: React.DragEvent, fish: FishAsset) => {
    ...
    const handleDrop = (e: React.DragEvent, slot: number) => {
        ...
        if(fishData) {
            ...
            const newPair: [FishAsset | null, FishAsset | null] = [...pair];
...
const SustainabilityReport: FC<{ onBack: () => void }> = ({ onBack }) => {
    const [reportType, setReportType] = useState('asc');
...
const App: FC = () => {
    ...
    const handleSelectTank = (tankId: number) => {
